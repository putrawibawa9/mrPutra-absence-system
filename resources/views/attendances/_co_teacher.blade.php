...
<div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
    ...
    <div id="co-teacher-rows" class="mt-3 space-y-2">
        @forelse ($coTeacherRows as $row)
            <div class="co-teacher-row flex flex-col gap-2 sm:flex-row sm:items-center">
                <select name="co_teacher_id[]" class="block w-full rounded-xl border-slate-300 text-sm sm:flex-1">
                    <option value="">— pilih guru —</option>
                    @foreach ($teachers as $teacher)
                        <option value="{{ $teacher->id }}" @selected((int) ($row['id'] ?? 0) === (int) $teacher->id)>{{ $teacher->name }}</option>
                    @endforeach
                </select>
                <input type="number" name="co_teacher_fee[]" min="0" value="{{ $row['fee'] ?? '' }}" placeholder="Salary (Rp)" class="block w-full rounded-xl border-slate-300 text-sm sm:w-48" />
        ...
        @empty
            <div class="co-teacher-row flex flex-col gap-2 sm:flex-row sm:items-center">
                ...
                <input type="number" name="co_teacher_fee[]" min="0" value="" placeholder="Salary (Rp)" class="block w-full rounded-xl border-slate-300 text-sm sm:w-48" />
                <button type="button" class="co-teacher-remove inline-flex justify-center rounded-xl border border-slate-300 px-3 py-2 text-xs font-medium text-slate-600">Hapus</button>
            </div>
        @endforelse
    </div>
